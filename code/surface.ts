...
class Surface{
    ...
    constructor(ni: number, nj: number, ti:number, tj:number, resolutioni: number, resolutionj: number){
        this.control_points = Array(ni+1).fill(null).map(() => Array(nj+1).fill(new Dot(Math.random()*30,Math.random()*30,Math.random()*30)))
        this.outp = Array(resolutioni).fill(null).map(() => Array(resolutionj).fill(new Dot(Math.random()*30,Math.random()*30,Math.random()*30)))
        this.ni = ni;
        this.nj = nj;
        this.ti = ti;
        this.tj = tj;
